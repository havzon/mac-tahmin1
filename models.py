...
class PredictionModel:
    ...
    def prepare_features(self, df, team1, team2):
        ...
        # Calculate form using new comprehensive function
    ...
    def predict_goals(self, features):
        """Predict the expected number of goals in the match"""
        home_goals = features[0][2]  # home team goals scored
        away_goals = features[0][4]  # away team goals scored
        home_conceded = features[0][3]  # home team goals conceded
        away_conceded = features[0][5]  # away team goals conceded

        # Calculate expected goals using historical averages and form
    ...
    def predict_over_under(self, features):
        """Predict over/under 2.5 goals probability"""
    ...
    def get_team_form(self, df, team):
        ...
        if len(recent_matches) == 0:
            return [0, 0, 0, 0, 0]  # Default values if no matches found
        ...
        form_metrics = [
            wins / num_matches,  # Win rate
            goals_scored / (num_matches * 3),  # Goals scored (normalized)
            clean_sheets / num_matches,  # Clean sheet rate
            (wins * 3) / (num_matches * 3),  # Form (based on points)
            (goal_diff + 5) / 10  # Goal difference (normalized)
        ]
